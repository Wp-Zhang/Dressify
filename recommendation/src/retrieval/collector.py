...
class RuleCollector:
    ...
    def collect(
        ...
        item_id : str, optional
            Item unit, by default ``"product_code"``.
        norm : bool, optional
        ...
        label = trans_df.loc[mask, ["customer_id", "product_code"]]
        label = label.drop_duplicates(["customer_id", "product_code"])
        label["label"] = 1
        ...
        for rule in tqdm(rules, "Retrieve items by rules"):
            ...
            if label.shape[0] != 0:
                ...
                tmp_items = tmp_items.merge(
                    label, on=["customer_id", "product_code"], how="left"
                )
